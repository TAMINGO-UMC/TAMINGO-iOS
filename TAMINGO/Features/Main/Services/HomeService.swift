    ...
    func fetchTodayTimeline(
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            if let http = response as? HTTPURLResponse {
                print("🌐 /api/home/today status:", http.statusCode)
            }

            // 원문 로그(너무 길면 잘라서)
            if let raw = String(data: data, encoding: .utf8) {
                print("📦 /api/home/today raw:\n", raw)
            }

            do {
                let decoded = try JSONDecoder().decode(HomeScheduleResponseDTO.self, from: data)
                completion(.success(decoded))
            } catch {
                print("❌ HomeSchedule decode error:", error)
                completion(.failure(error))
            }
        }
        .resume()
