    ...
    var body: some View {
        ScrollView{
            VStack(alignment:.leading){
                ...
                CategorySection(vm: vm, onTapSchedule: {
                    onSelect(.scheduleCategory)
                }, onTapTodo: {
                    onSelect(.todoCategory)
                })
                SyncSection(vm: vm, onTap: {
        ...
        SectionContainerView(title: "카테고리 설정"){
            VStack(spacing: 0) {
                CategorySettingRowView(
                    title: "일정 카테고리",
                    sub: vm.scheduleCategoryCountText,
                    textColor: .gray2
                ) {
                    onTapSchedule()
                }
