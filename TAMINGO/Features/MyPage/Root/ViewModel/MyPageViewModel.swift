...
// 임시
private var defaultWeeklyMetrics: [WeeklyMetric] {
    [
        WeeklyMetric(
            title: "정시 도착률",
            value: "-%",
...
final class MyPageViewModel {
    ...
    var weeklyMetrics: [WeeklyMetric] {
        guard let report = myPage?.weeklyReport else {
            return defaultWeeklyMetrics
        }

        let onTimeRateText = report.onTimeRate.map { "\($0)%" } ?? "-%"

        let onTimeDiffText = report.onTimeDiff.map {
            $0 == 0 ? "변동 없음" :
            $0 > 0 ? "+\($0)% 상승" :
            "\($0)% 감소"
        } ?? "-"
        ...
        let productivityText = report.productivityScore.map { "\($0)점" } ?? "-점"
        ...
        return [
            WeeklyMetric(
                title: "정시 도착률",
                value: onTimeRateText,
                subValue: onTimeDiffText,
                iconName: "MyPage_icon_report01",
                textColor: .subBlue2,
                backgroundColor: .subBlue1
            ),
    ...
    // 카테고리
    var scheduleCategoryCountText: String {
        let count = myPage?.counts.scheduleCategoryCount ?? 0
        return "\(count)개"
    }

    var todoCategoryCountText: String {
        let count = myPage?.counts.todoCategoryCount ?? 0
        return "\(count)개"
    }
